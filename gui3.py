...
image=ImageTk.PhotoImage(Image.open("backgroung.jpg"))
...
INQUIRY = Button(text="LOGIN", width=17, font=("arial", 20), bg="chocolate3", command=login)
INQUIRY.place(x=500, y=380)
...
def DisplayForm():
    ...
    LFrom = Frame(firstw, width="350", bg="royalblue")
    LFrom.pack(side=LEFT, fill=Y)
    # seconf left frame for search form
    LeftViewForm = Frame(firstw, width=500, bg="cadetblue")
    LeftViewForm.pack(side=LEFT, fill=Y)
    # mid frame for displaying students record
    MidViewForm = Frame(firstw, width=600, )
    MidViewForm.pack(side=RIGHT)
    # label for heading
    lbl_text = Label(TopViewForm, text="Student Registration System", font=('Kalinga', 22, "bold", "italic"), width=600,
                     bg="palevioletred",
                     fg="ivory")
    lbl_text.pack(fill=X)
    # creating registration form in first left frame
    Label(LFrom, text="Name  ", font=("Arial", 16, "bold"), bg="royalblue").pack(side=TOP)
    Entry(LFrom, font=("Arial", 14, "bold"), textvariable=name).pack(side=TOP, padx=10, fill=X)
    Label(LFrom, text="Contact ", font=("Arial", 16, "bold"), bg="royalblue").pack(side=TOP)
    Entry(LFrom, font=("Arial", 14, "bold"), textvariable=contact).pack(side=TOP, padx=10, fill=X)
    Label(LFrom, text="Email ", font=("Arial", 16, "bold"), bg="royalblue").pack(side=TOP)
    Entry(LFrom, font=("Arial", 14, "bold"), textvariable=email).pack(side=TOP, padx=10, fill=X)
    Label(LFrom, text="Rollno ", font=("Arial", 16, "bold"), bg="royalblue").pack(side=TOP)
    Entry(LFrom, font=("Arial", 14, "bold"), textvariable=rollno).pack(side=TOP, padx=10, fill=X)
    Label(LFrom, text="Branch ", font=("Arial", 16, "bold"), bg="royalblue").pack(side=TOP)
    Entry(LFrom, font=("Arial", 14, "bold"), textvariable=branch).pack(side=TOP, padx=10, fill=X)
    Button(LFrom, text="Submit", font=("Arial", 16, "bold"), bg="cornsilk", command=register).pack(side=TOP, padx=10,
                                                                                                   pady=5, fill=X)
